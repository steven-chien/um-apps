...
edgelist_to_CSR(const struct packed_edge * restrict IJ, const int64_t nedge)
{
	...
	HANDLE_ERROR(cudaMemPrefetchAsync(d_CSR_R, sizeof(int32_t)*(nv+1), 0, s1));
	HANDLE_ERROR(cudaMemPrefetchAsync(d_CSR_C, sizeof(int32_t)*maxedg, 0, s1));
...
//	HANDLE_ERROR(cudaMemcpy(d_n_in_queue,&h_n_in_queue,sizeof(int32_t),cudaMemcpyHostToDevice));
	...
	HANDLE_ERROR(cudaMemPrefetchAsync(d_predecessors, sizeof(int32_t)*nv, 0, s1));
	HANDLE_ERROR(cudaMemPrefetchAsync(d_in_queue, sizeof(BITMAP_TYPE)*nwords, 0, s1));
	HANDLE_ERROR(cudaMemPrefetchAsync(d_visited_tex, sizeof(BITMAP_TYPE)*nwords, 0, s1));
...
//	HANDLE_ERROR(cudaMemcpy(h_predecessors,d_predecessors,sizeof(int32_t)*nv,cudaMemcpyDeviceToHost));
	HANDLE_ERROR(cudaMemPrefetchAsync(h_predecessors, sizeof(int32_t)*nv, cudaCpuDeviceId));
...
void destroy_graph()
{
	...
	HANDLE_ERROR(cudaStreamDestroy(s1));
}
