...
edgelist_to_CSR(const struct packed_edge * restrict IJ, const int64_t nedge)
{
	...
	HANDLE_ERROR(cudaMalloc((void**)&d_tex_visited,sizeof(BITMAP_TYPE)*nwords)); 	
...
__device__ void CTAScan( volatile int * input, volatile int * output, int n)
{
	__shared__ int temp[NWARPS*32];
	int thx = threadIdx.x; 
	int offset = 1; 
	temp[2*thx] = input[2*thx]; 
	temp[2*thx+1] = input[2*thx+1]; 
	for(int d = n>>1 ; d > 0 ; d>>=1)
	{
		__syncthreads(); 
		if(thx < d)
		{
			int ai = offset * (2*thx+1)-1;
			int bi = offset * (2*thx+2)-1;
			temp[bi] += temp[ai]; 
		}
		offset*=2; 
	}
	...
	__syncthreads(); 
	output[2*thx] = temp[2*thx]; 
	output[2*thx+1] =temp[2*thx+1]; 
}
...
__device__ int history_culling(int32_t neighbor, volatile int * cache)
{
	int hash = neighbor & 2047;
	int retrieved = cache[hash]; 
	//__syncthreads();
	//printf("thx(%d) vtx(%d) load(%d)\n",threadIdx.x,neighbor,retrieved);
	if(retrieved == neighbor)
		return 1; 
	cache[hash] = neighbor; 
	return 0; 
...
__device__ int labelLookup(int32_t * predecessors, int32_t neighbor, int32_t vtx )
{
...
__device__ int bitmapLookup(int32_t * predecessors, int32_t neighbor, int32_t vtx, BITMAP_TYPE * b_tex_visited)
{
	int32_t mask_offset = neighbor / BITMAP_WORD; 
	BITMAP_TYPE mask = tex1Dfetch(tex_visited,mask_offset);
	BITMAP_TYPE mask_bit = 1 << (neighbor % BITMAP_WORD); 
	volatile int32_t visited; 
	if(mask & mask_bit)
	{
		return 0;
	}else{
		//atomicOr(&(b_tex_visited[mask_byte_offset]),mask_bit); 
		b_tex_visited[mask_offset] |=  mask_bit; 
		visited = (predecessors[neighbor] != -1); 
		if(!visited)
			return 1;
		return 0;
	}
...
__global__ void expand_contract_merrill(	int32_t * CSR_R, int32_t * CSR_C,
						...
						int32_t * predecessors, BITMAP_TYPE * b_tex_visited)
{
	// Global thread id 
	...
	while(__any(range[2]))
	{
		...
		{
			// GLOBAL MEMORY access
			volatile int32_t neighbor = CSR_C[r_gather];	
			int sum = 0; 
			int active = labelLookup(predecessors,neighbor,shared_vertex[warp_id]); 	
			//int active = bitmapLookup(predecessors,neighbor,shared_vertex[warp_id],b_tex_visited);
...
#else
	// CTA Wide version
	volatile __shared__ int comm[NWARPS*32]; 
	volatile __shared__ int prefixScan[NWARPS*32]; 
	prefixScan [threadIdx.x] = range[2]; 
	// Check if there is work to do with a prefix scan 
 	CTAScan(prefixScan,prefixScan,NWARPS*32);
	int total = prefixScan[NWARPS*32-1]; 
	int rsv_rank = prefixScan[threadIdx.x];
       	int remain = 0;
	//int r_gather, r_gather_end;	
	__shared__ int offset_CTA;
        offset_CTA = 0; 	

	int cta_progress = 0; 
	while((remain = total - cta_progress) > 0)
	{
		prefixScan[threadIdx.x] = 0; 
		while((rsv_rank < cta_progress + blockDim.x) && (range[0] < range[1]))
		{
			comm[rsv_rank - cta_progress] = range[0]; 
			rsv_rank++; 
			range[0]++; 
		}
		__syncthreads(); 
		if(threadIdx.x < min(remain,blockDim.x))
		{
			int32_t neighbor = CSR_C[comm[threadIdx.x]]; 
			// Check and add to out queue 
			int active = labelLookup(predecessors,neighbor,vtx); 	
			//int active = bitmapLookup(predecessors,neighbor,shared_vertex[warp_id],b_tex_visited);
			
			if(active)
				// GLOBAL MEMORY access
				predecessors[neighbor] = vtx; 

			prefixScan[threadIdx.x] = active; 
			// Prefix sum to know if the verticies are working or not 
			CTAScan(prefixScan,prefixScan,NWARPS*32);
			int offset_thread = prefixScan[threadIdx.x]; 
			int sum = prefixScan[NWARPS*32-1];
			if(threadIdx.x == 0)
				offset_CTA = atomicAdd(n_out_queue,sum); 

			if(active && offset_thread > -1)
				out_queue[offset_CTA + offset_thread] = neighbor; 
		}
...
__global__ void setup_GPU(int32_t * predecessors, int64_t srcvtx, int32_t * in_queue, BITMAP_TYPE * b_tex_visited)
{
	predecessors[srcvtx] = (int32_t)srcvtx;
	in_queue[0] = srcvtx;
	b_tex_visited[srcvtx/BITMAP_WORD] = srcvtx % BITMAP_WORD; 
}
...
int make_bfs_tree( int64_t *bfs_tree_out, int64_t *max_vtx_out, int64_t srcvtx)
{
	...
	*max_vtx_out = maxvtx;
	h_n_in_queue = 1;
	HANDLE_ERROR(cudaMemset(d_tex_visited,0,sizeof(BITMAP_TYPE)*nwords)); 
	...
	setup_GPU<<<1,1>>>(d_predecessors,srcvtx,d_in_queue,d_tex_visited);
	...
	while(1)
	{
		...
		HANDLE_ERROR(cudaBindTexture(0, tex_visited,d_tex_visited, sizeof(BITMAP_TYPE)*nwords));
		printf(" CSR ");
		...
		if(iteration%2)
		{
			...
			expand_contract_merrill<<< dimGrid.x,dimBlock.x >>>(	
					...
					d_out_queue,d_n_out_queue,d_predecessors,d_tex_visited);
		}else{
			printf(" C2 ");
			expand_contract_merrill<<< dimGrid.x,dimBlock.x >>>(	
					d_CSR_R,d_CSR_C,d_out_queue,d_n_in_queue,
					d_in_queue,d_n_out_queue,d_predecessors,d_tex_visited);
		}
