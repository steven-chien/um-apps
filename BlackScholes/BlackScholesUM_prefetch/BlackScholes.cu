...
//    checkCudaErrors(cudaMemcpy(d_OptionYears,  h_OptionYears,   OPT_SZ, cudaMemcpyHostToDevice));
    ...
    checkCudaErrors(cudaMemPrefetchAsync(h_StockPrice, OPT_SZ, devID, s1));
    checkCudaErrors(cudaMemPrefetchAsync(h_OptionStrike, OPT_SZ, devID, s1));
    checkCudaErrors(cudaMemPrefetchAsync(h_OptionYears, OPT_SZ, devID, s1));

    checkCudaErrors(cudaMemPrefetchAsync(h_CallResultGPU, OPT_SZ, devID, s1));
    checkCudaErrors(cudaMemPrefetchAsync(h_PutResultGPU, OPT_SZ, devID, s1));
